  ...
  if (section === 'valuation' || section === 'market') {
    ...
    return answer(
      strictValue
        ...
        : areaReference
          ? (isSpanish
              ? `Abrí la evidencia de mercado de “${title}”. Por ahora existe una referencia orientativa de precios anunciados de ${formatMoney(areaReference, valuation?.currency || 'MXN', language)}, pero los datos todavía no cumplen el nivel requerido para llamarla estimación de valor. ¿Quieres revisar el rango y las advertencias?`
              : `I opened the market evidence for “${title}”. For now there is an indicative asking-price reference of ${formatMoney(areaReference, valuation?.currency || 'MXN', language)}, but the data does not yet meet the threshold for a market-value estimate. Would you like to review the range and warnings?`)
          : (isSpanish
