                  ...
                  {form.offerings.map((offering) => (
                    <div key={offering.mode} className="rounded-3xl border border-slate-200 bg-slate-50/70 p-5">
                      ...
                      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                        ...
                        {offering.mode !== 'SWAP' && <Field label="Comisión total (%)" hint="Porcentaje pactado con el propietario."><input type="number" min="0" max="100" step="0.1" value={offering.commissionTotalPct ?? ''} onChange={(event) => updateOffering(offering.mode, { commissionTotalPct: event.target.value === '' ? null : Number(event.target.value) })} className={inputClass} /></Field>}
                        {offering.mode !== 'SWAP' && <Field label="Comisión compartida (%)" hint="Porcentaje para el asesor cooperador."><input type="number" min="0" max="100" step="0.1" value={offering.commissionSharedPct ?? ''} onChange={(event) => updateOffering(offering.mode, { commissionSharedPct: event.target.value === '' ? null : Number(event.target.value) })} className={inputClass} /></Field>}
