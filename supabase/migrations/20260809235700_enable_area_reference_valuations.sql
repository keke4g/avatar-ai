-- Towers Market v5 keeps distance-verified estimates and neighborhood asking-
-- price references as two separate products. AREA_REFERENCE never populates
-- estimated market-value, comparison, yield or cap-rate fields.

alter table valuation.property_valuation_runs
  add column if not exists evidence_tier text not null default 'INSUFFICIENT'
    check (evidence_tier in ('STRICT_ESTIMATE', 'AREA_REFERENCE', 'INSUFFICIENT')),
  add column if not exists area_reference_value numeric,
  add column if not exists area_range_low numeric,
  add column if not exists area_range_high numeric,
  add column if not exists area_price_per_m2 numeric,
  add column if not exists area_reference_operation text
    check (area_reference_operation is null or area_reference_operation in ('SALE', 'MONTHLY_RENT')),
  add column if not exists area_location_basis text
    check (area_location_basis is null or area_location_basis = 'NEIGHBORHOOD');

alter table valuation.property_valuation_comparables
...
begin
  ...
  v_area_valid := (
    ...
    and new.confidence_score between 45 and 64
    and new.comparable_count >= 10
    and v_source_count >= 2
  ...
  if new.status <> 'COMPLETED' or not (v_strict_valid or v_area_valid) then
    ...
    if v_was_completed then
      new.warnings := array_append(coalesce(new.warnings, '{}'::text[]), 'La corrida no superó la compuerta pública Towers Market v5.');
    end if;
  end if;
  return new;
end;
...
begin
  ...
  if v_actual_status = 'COMPLETED' and jsonb_typeof(v_valuation -> 'comparables') = 'array' then
    ...
    if v_comparable_count <> v_expected_count or v_source_count < 2 then
      update valuation.property_valuation_runs
      set status = 'INSUFFICIENT_DATA',
          warnings = array_append(warnings, 'Los comparables persistidos no coinciden con la muestra validada o sus fuentes.')
      where id = v_run_id
...
where run.status = 'COMPLETED'
  ...
  and run.comparable_count = (
    select count(*) from valuation.property_valuation_comparables saved
    where saved.valuation_run_id = run.id
  )
  and 2 <= (
    select count(distinct nullif(saved.adjustments ->> 'sourceCode', ''))
  ...
  and (
    ...
    (
      ...
      and run.confidence_score between 45 and 64
      and run.comparable_count >= 10
...
comment on view public.public_property_valuations_view is
  'Newest Towers Market v5 result only: either a distance-verified estimate or a separately labeled asking-price area reference.';
