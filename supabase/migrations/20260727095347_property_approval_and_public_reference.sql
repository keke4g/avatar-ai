...
begin
  -- SQL migrations and service-role jobs have no end-user identity. They may
  -- preserve an explicitly supplied moderation state.
  ...
  -- The public folio and owner are immutable after creation.
  new.internal_code := old.internal_code;
  new.host_id := old.host_id;

  if not actor_is_admin then
    -- Every owner edit goes back through moderation. This prevents a member
    -- from publishing directly or changing an already-approved listing
