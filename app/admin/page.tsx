...
import { 
  ...
  MapPin, UserCheck, AlertCircle, Check, DollarSign, Calendar, Volume2, Radio, Zap
} from 'lucide-react';
                  ...
                  <div className="flex flex-col gap-4 pt-5 border-t border-brand-gray-100 mt-1">
                      ...
                      {ETERNA_VOICE_ENGINES.map((engine) => {
                        ...
                        return (
                          <button
                            key={engine.id}
                            type="button"
                            onClick={() => setVoiceEngine(engine.id)}
                          ...
                          >
                            ...
                            <div className="pr-5">
                              <div className="flex items-center gap-1.5 mb-2">
                                {engine.id === 'elevenlabs' ? <Zap className="w-3.5 h-3.5 text-brand-accent" /> : <Volume2 className="w-3.5 h-3.5 text-brand-gray-500" />}
                            ...
                            <div className="mt-3 flex flex-wrap items-center gap-1.5">
                              <span className="px-2 py-1 rounded-full bg-white border border-brand-gray-200 text-[8px] font-black uppercase tracking-wide text-brand-gray-500">
                                {engine.badge}
                              </span>
                    ...
                    {voiceEngine === 'azure' && !voiceEngineStatus.azure && (
                      <div className="rounded-xl border border-amber-200 bg-amber-50/70 px-3 py-2.5 text-[10px] font-semibold leading-relaxed text-amber-800">
                        Para activar Dalia Neural agrega <code>AZURE_SPEECH_KEY</code> y <code>AZURE_SPEECH_REGION</code> en Vercel. Mientras tanto Eterna usará automáticamente la voz del navegador.
                      </div>
                    )}
                  </div>
