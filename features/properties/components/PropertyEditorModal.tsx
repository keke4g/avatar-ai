...
import ImageUploadDropzone, { ImageMetadata } from '@/components/ImageUploadDropzone';
...
type EditorForm = {
  ...
  legalDebtFree: boolean | null;
  legalPublicDeed: boolean | null;
  legalTaxCurrent: boolean | null;
  legalServicesPaid: boolean | null;
  legalOwnerType: string;
  legalIsMortgaged: boolean | null;
...
const NAV_ITEMS: Array<{
  ...
  { id: 'spaces', label: 'Medidas', caption: 'Superficies y capacidad', icon: SlidersHorizontal },
  { id: 'amenities', label: 'Espacios + amenidades', caption: 'Privadas y compartidas', icon: Sparkles },
...
function formFromProperty(property: Property): EditorForm {
  ...
  return {
    ...
    legalDebtFree: property.legalDebtFree ?? null,
    legalPublicDeed: property.legalPublicDeed ?? null,
    legalTaxCurrent: property.legalTaxCurrent ?? null,
    legalServicesPaid: property.legalServicesPaid ?? null,
    legalOwnerType: property.legalOwnerType || '',
    legalIsMortgaged: property.legalIsMortgaged ?? null,
    ...
    videoUrls: [
      ...(property.media || []).filter((item) => item.mediaType === 'VIDEO').map((item) => item.url),
      ...((property as Property & { videoUrl?: string }).videoUrl ? [String((property as Property & { videoUrl?: string }).videoUrl)] : []),
    ].filter((url, index, list) => Boolean(url) && list.indexOf(url) === index).slice(0, 5),
    ...
    folderStatus: property.folderStatus || (property.isPublished === true ? 'PUBLISHED' : 'DRAFT'),
...
function defaultOffering(property: Property, mode: PropertyOfferingMode): PropertyOffering {
  return {
    ...
    isFeatured: false,
    featuredRank: 0,
    depositAmount: mode === 'MONTHLY_RENT' ? 0 : null,
    securityDepositAmount: mode === 'MONTHLY_RENT' ? 0 : null,
    advanceMonths: mode === 'MONTHLY_RENT' ? 1 : undefined,
    minMonths: mode === 'MONTHLY_RENT' ? 12 : null,
    requiresGuarantor: mode === 'MONTHLY_RENT' ? false : undefined,
    requiresLegalPolicy: mode === 'MONTHLY_RENT' ? false : undefined,
    metadata: mode === 'MONTHLY_RENT'
      ? {
          rentalFurnishingStatus: 'UNFURNISHED',
          includedRentalServices: [],
          includesMaintenance: false,
          acceptsPets: false,
          rentalCommissionModel: 'ONE_MONTH_RENT',
          rentalCommissionMonths: 1,
          rentalCommissionAmount: null,
        }
      : {},
  };
}
...
export default function PropertyEditorModal({ isOpen, property, onClose, onSubmit, onDelete }: PropertyEditorModalProps) {
  ...
  const updateMonthlyRental = (
    offeringPatch: Partial<PropertyOffering>,
    ...
    setForm((current) => ({
      ...
      offerings: current.offerings.map((offering) => (
        offering.mode === 'MONTHLY_RENT'
          ? {
              ...offering,
              ...offeringPatch,
              metadata: { ...(offering.metadata || {}), ...metadataPatch },
            }
          : offering
      )),
    }));
  ...
  const toggleIncludedRentalService = (service: 'WATER' | 'ELECTRICITY' | 'INTERNET') => {
    setSaved(false);
    setForm((current) => {
      const monthlyOffering = current.offerings.find((offering) => offering.mode === 'MONTHLY_RENT');
      const storedServices = current.metadata.includedRentalServices
        ?? monthlyOffering?.metadata?.includedRentalServices;
      const services = Array.isArray(storedServices)
        ? storedServices.filter((item): item is string => typeof item === 'string')
        : [];
      const nextServices = services.includes(service)
        ? services.filter((item) => item !== service)
        : [...services, service];
      const metadataPatch = {
        includedRentalServices: nextServices,
        includesServices: nextServices.length > 0,
      };
      ...
      return {
        ...
        offerings: current.offerings.map((offering) => (
          offering.mode === 'MONTHLY_RENT'
            ? { ...offering, metadata: { ...(offering.metadata || {}), ...metadataPatch } }
            : offering
        )),
      };
    });
  ...
  const buildMedia = (): Partial<PropertyMedia>[] => {
    ...
    form.videoUrls.slice(0, 5).forEach((videoUrl) => {
      if (videoUrl.trim()) media.push({ mediaType: 'VIDEO', url: videoUrl.trim(), displayOrder: order++, isPrimary: false, metadata: {} });
    });
  ...
  const handleSave = async () => {
    ...
    const sensitiveSnapshot = (value: EditorForm) => JSON.stringify({
      title: value.title,
      ...
      images: value.images,
      legal: [value.legalDebtFree, value.legalPublicDeed, value.legalTaxCurrent, value.legalServicesPaid],
    });
    const requiresReview = property.isPublished === true
      && sensitiveSnapshot(form) !== sensitiveSnapshot(initialForm);
    const canRemainPublished = isAdmin
      && form.isPublished
      && form.folderStatus === 'PUBLISHED'
      && !requiresReview;
    const targetFolderStatus = requiresReview || !isAdmin
      ? 'UNDER_REVIEW'
      : form.folderStatus;
    const normalizedOfferings = offerings.map((offering) => ({
      ...offering,
      status: (canRemainPublished
        ? 'ACTIVE'
        : targetFolderStatus === 'UNDER_REVIEW'
          ? 'DRAFT'
          : 'PAUSED') as PropertyOffering['status'],
    }));
    ...
    const patch: Partial<Property> = {
      ...
      metadata: {
        ...form.metadata,
        imagesMetadata: form.imagesMetadata,
      },
      isPublished: canRemainPublished,
      folderStatus: targetFolderStatus,
    ...
    const validation = PropertyValidator.validatePropertyBeforeInsert(
      { ...property, ...patch, images: patch.images || property.images },
      canRemainPublished ? 'PUBLICATION' : targetFolderStatus === 'DRAFT' ? 'DRAFT' : 'REVIEW',
    );
    if (!validation.success) {
      setSaveError(validation.errors[0]?.message || 'La propiedad no cumple el control de calidad.');
      const firstField = validation.errors[0]?.field || '';
      if (firstField.includes('latitude') || firstField.includes('longitude') || firstField === 'location') {
        scrollToSection('location');
      } else if (firstField === 'images') {
        scrollToSection('media');
      } else if (firstField.startsWith('offerings')) {
        scrollToSection('commercial');
      }
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-[150] bg-slate-950/55 p-0 backdrop-blur-md sm:p-5">
            ...
            <div className="overflow-hidden rounded-2xl border border-white/10 bg-white/5">
              <div className="relative aspect-[16/9] bg-slate-900">
                {form.images[0] ? (
                  ...
                  <div className="flex h-full items-center justify-center"><ImageIcon className="h-7 w-7 text-slate-600" /></div>
                )}
              </div>
              <div className="p-3.5">
            ...
            <div className="mx-auto flex max-w-[1040px] flex-col gap-5 pb-28">
              <Section id="general" eyebrow="01 · Identidad" title="Información principal" description="Estos datos construyen la portada pública del anuncio y la presentación que utiliza Eterna.">
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <Field label="Categoría Towers"><select value={form.valueRating} onChange={(event) => update('valueRating', event.target.value as Property['valueRating'])} className={inputClass}>{['Premium', 'Luxury', 'Exclusive', 'Curated'].map((option) => <option key={option}>{option}</option>)}</select></Field>
              ...
              <Section id="amenities" eyebrow="04 · Experiencia" title="Espacios y amenidades" description="Distingue los espacios propios del inmueble, el equipamiento privado y los servicios compartidos del desarrollo.">
                <div className="mb-7 grid gap-4 xl:grid-cols-3">
                  {PROPERTY_FEATURE_GROUPS.map((group, groupIndex) => (
                    <div key={group.key} className="rounded-[24px] border border-slate-200 bg-slate-50/70 p-4">
                      <div className="mb-3 flex items-start gap-3">
                        <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-slate-950 text-[9px] font-black text-white">0{groupIndex + 1}</span>
                        <div>
                          <h4 className="text-[11px] font-black text-slate-950">{group.titleEs}</h4>
                          <p className="mt-0.5 text-[9px] leading-relaxed text-slate-500">{group.descriptionEs}</p>
                        </div>
                      </div>
                        ...
                        {group.options.map((amenity) => {
                          ...
                          return (
                            <button
                              ...
                              className={`flex items-center gap-1 rounded-full border px-2.5 py-1.5 text-[9px] font-extrabold transition ${
                                active
                            ...
                            >
                              <AmenityIcon className="h-3 w-3" aria-hidden="true" />
                ...
                {form.amenities.some((amenity) => !AMENITY_OPTIONS.includes(amenity as never)) && (
                  <div className="mb-7 rounded-2xl border border-dashed border-slate-300 bg-white p-4">
                    <p className="mb-2 text-[10px] font-black uppercase tracking-[0.12em] text-slate-500">Características personalizadas</p>
                    <div className="flex flex-wrap gap-2">
                      {form.amenities
                        .filter((amenity) => !AMENITY_OPTIONS.includes(amenity as never))
                        .map((amenity) => (
                          <button key={amenity} type="button" onClick={() => toggleAmenity(amenity)} className="flex items-center gap-1 rounded-full border border-violet-200 bg-violet-50 px-3 py-2 text-[9px] font-extrabold text-violet-700">
                ...
                <div className="mb-3">
                  <h4 className="text-xs font-black text-slate-950">Servicios del inmueble</h4>
                  <p className="mt-0.5 text-[10px] text-slate-500">Infraestructura disponible y reglas operativas.</p>
                </div>
                ...
                <div className="mb-3 mt-7">
                  <h4 className="text-xs font-black text-slate-950">Seguridad y accesos</h4>
                  <p className="mt-0.5 text-[10px] text-slate-500">Controles instalados en la propiedad o el desarrollo.</p>
                </div>
              ...
              <Section id="commercial" eyebrow="05 · Operación" title="Modalidades comerciales" description="Activa únicamente las formas en que deseas ofrecer la propiedad. Los datos existentes de cada modalidad se conservan.">
                ...
                <div className="mt-4">
                  <Toggle
                    checked={form.metadata.isExclusive === true}
                    onChange={(value) => update('metadata', { ...form.metadata, isExclusive: value })}
                    label="Ficha en exclusiva"
                    description="Confirma que Towers México cuenta con autorización exclusiva para comercializarla."
                  />
                </div>
                <div className="mt-5 flex flex-col gap-4">
                  {form.offerings.map((offering) => {
                    const rentalMetadata = offering.mode === 'MONTHLY_RENT'
                      ? { ...(offering.metadata || {}), ...form.metadata }
                      : {};
                    const rentalFurnishingStatus = (
                      rentalMetadata.rentalFurnishingStatus || 'UNFURNISHED'
                    ) as 'UNFURNISHED' | 'SEMI_FURNISHED' | 'FURNISHED';
                    const includedRentalServices = Array.isArray(rentalMetadata.includedRentalServices)
                      ? rentalMetadata.includedRentalServices.filter((item): item is string => typeof item === 'string')
                      : [];
                    const rentalCommissionModel = rentalMetadata.rentalCommissionModel === 'PERCENTAGE'
                      ? 'PERCENTAGE'
                      : 'ONE_MONTH_RENT';
                    const rentalRequiresGuarantor = typeof rentalMetadata.requiresGuarantor === 'boolean'
                      ? rentalMetadata.requiresGuarantor
                      : offering.requiresGuarantor === true;
                    const rentalRequiresLegalPolicy = typeof rentalMetadata.requiresLegalPolicy === 'boolean'
                      ? rentalMetadata.requiresLegalPolicy
                      : offering.requiresLegalPolicy === true;

                    return (
                    <div key={offering.mode} className="rounded-3xl border border-slate-200 bg-slate-50/70 p-4 sm:p-5">
                      ...
                      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                        {offering.mode !== 'SWAP' && <Field label={offering.mode === 'SALE' ? 'Precio de venta' : offering.mode === 'MONTHLY_RENT' ? 'Renta mensual' : 'Precio por noche'}><input type="number" min="0" value={offering.priceAmount ?? ''} onChange={(event) => {
                          const priceAmount = event.target.value === '' ? null : Number(event.target.value);
                          if (offering.mode === 'MONTHLY_RENT') {
                            const currentDeposit = offering.securityDepositAmount ?? offering.depositAmount ?? 0;
                            const shouldSyncOneMonthDeposit = shouldSyncSuggestedRentalDeposit(
                              currentDeposit,
                            ...
                            updateMonthlyRental(
                              {
                                priceAmount,
                                ...(shouldSyncOneMonthDeposit
                                  ? {
                                      securityDepositAmount: priceAmount,
                                      depositAmount: priceAmount,
                                    }
                                  : {}),
                              },
                              rentalCommissionModel === 'ONE_MONTH_RENT'
                                ? { rentalCommissionAmount: priceAmount, rentalCommissionMonths: 1 }
                                : {},
                            );
                          } else {
                            updateOffering(offering.mode, { priceAmount });
                          }
                        ...
                        {offering.mode !== 'SWAP' && (offering.mode !== 'MONTHLY_RENT' || rentalCommissionModel === 'PERCENTAGE') && <Field label={offering.mode === 'SALE' ? 'Captación total (%)' : 'Comisión total (%)'} hint="Porcentaje total pactado con el propietario, por ejemplo 5% o 6%."><input type="number" min="0" max="100" step="0.1" value={offering.commissionTotalPct ?? ''} onChange={(event) => updateOffering(offering.mode, { commissionTotalPct: event.target.value === '' ? null : Number(event.target.value) })} className={inputClass} /></Field>}
                        {offering.mode !== 'SWAP' && (offering.mode !== 'MONTHLY_RENT' || rentalCommissionModel === 'PERCENTAGE') && <Field label="Comisión compartida (%)" hint="Porcentaje para el asesor cooperador."><input type="number" min="0" max="100" step="0.1" value={offering.commissionSharedPct ?? ''} onChange={(event) => updateOffering(offering.mode, { commissionSharedPct: event.target.value === '' ? null : Number(event.target.value) })} className={inputClass} /></Field>}
                        ...
                        {offering.mode === 'SHORT_RENT' && <Field label="Precio semanal"><input type="number" min="0" value={typeof offering.metadata?.weeklyPrice === 'number' ? offering.metadata.weeklyPrice : ''} onChange={(event) => updateOffering(offering.mode, { metadata: { ...offering.metadata, weeklyPrice: event.target.value === '' ? null : Number(event.target.value) } })} className={inputClass} /></Field>}
                        ...
                        {offering.mode === 'SHORT_RENT' && <Field label="Depósito"><input type="number" min="0" value={offering.securityDepositAmount ?? ''} onChange={(event) => updateOffering(offering.mode, { securityDepositAmount: event.target.value === '' ? null : Number(event.target.value) })} className={inputClass} /></Field>}
                        {offering.mode === 'MONTHLY_RENT' && <Field label="Depósito de garantía" hint="Se propone un mes de renta por defecto; puedes cambiarlo o dejarlo en $0."><input type="number" min="0" value={offering.securityDepositAmount ?? offering.depositAmount ?? ''} onChange={(event) => {
                          const deposit = event.target.value === '' ? null : Number(event.target.value);
                          updateMonthlyRental({ securityDepositAmount: deposit, depositAmount: deposit });
                        }} className={inputClass} /></Field>}
                        {offering.mode === 'MONTHLY_RENT' && <Field label="Meses adelantados"><input type="number" min="0" value={offering.advanceMonths ?? ''} onChange={(event) => {
                          const advanceMonths = event.target.value === '' ? undefined : Number(event.target.value);
                          updateMonthlyRental({ advanceMonths }, { advanceMonths: advanceMonths ?? null });
                        }} className={inputClass} /></Field>}
                      ...
                      {offering.mode === 'SALE' && (
                        <div className="mt-5 border-t border-slate-200 pt-5">
                          <p className="text-[11px] font-black text-slate-950">Formas de pago y negociación</p>
                          <p className="mt-1 text-[10px] text-slate-500">Mantén actualizadas las opciones que verá el comprador.</p>
                          <div className="mt-3 grid gap-3 md:grid-cols-2">
                            <Toggle checked={offering.acceptsOffers === true} onChange={(value) => updateOffering('SALE', { acceptsOffers: value })} label="Acepta ofertas" />
                            <Toggle checked={offering.acceptsCash === true} onChange={(value) => updateOffering('SALE', { acceptsCash: value })} label="Contado" />
                            <Toggle checked={offering.acceptsBankCredit === true} onChange={(value) => updateOffering('SALE', { acceptsBankCredit: value })} label="Crédito bancario" />
                            <Toggle checked={offering.acceptsInfonavit === true} onChange={(value) => updateOffering('SALE', { acceptsInfonavit: value })} label="Infonavit" />
                            <Toggle checked={offering.acceptsFovissste === true} onChange={(value) => updateOffering('SALE', { acceptsFovissste: value })} label="Fovissste" />
                            <Toggle checked={offering.developerFinancing === true} onChange={(value) => updateOffering('SALE', { developerFinancing: value })} label="Financiamiento directo" />
                          </div>
                        </div>
                      )}
                      {offering.mode === 'MONTHLY_RENT' && (
                        <div className="mt-5 space-y-5 border-t border-slate-200 pt-5">
                          <div>
                            <p className="text-[11px] font-black text-slate-950">Condiciones de contratación</p>
                            <p className="mt-1 text-[10px] text-slate-500">Estos datos aparecen en el resumen público de la renta.</p>
                          </div>
                          <div className="grid gap-3 md:grid-cols-2">
                            <Toggle checked={rentalMetadata.requiresContract === true} onChange={(value) => updateMonthlyRental({}, { requiresContract: value })} label="Contrato requerido" description="Indica si se firma contrato formal de arrendamiento." />
                            <Toggle checked={rentalRequiresGuarantor} onChange={(value) => updateMonthlyRental({ requiresGuarantor: value }, { requiresGuarantor: value })} label="Aval u obligado solidario" description="Indica si es indispensable para firmar." />
                            <Toggle checked={rentalRequiresLegalPolicy} onChange={(value) => updateMonthlyRental({ requiresLegalPolicy: value }, { requiresLegalPolicy: value })} label="Póliza jurídica" description="Actívala si forma parte de los requisitos." />
                            <Toggle checked={rentalMetadata.acceptsPets === true} onChange={(value) => updateMonthlyRental({}, { acceptsPets: value })} label="Acepta mascotas" />
                            <Toggle checked={rentalMetadata.includesMaintenance === true} onChange={(value) => updateMonthlyRental({}, { includesMaintenance: value })} label="Mantenimiento incluido" />
                          </div>

                          <fieldset>
                            <legend className="text-[11px] font-extrabold text-slate-700">Estado del mobiliario</legend>
                            <div className="mt-2 grid gap-2 sm:grid-cols-3">
                              {([
                                ...
                                <button
                                  key={value}
                                  type="button"
                                  aria-pressed={rentalFurnishingStatus === value}
                                  onClick={() => updateMonthlyRental({}, {
                                    rentalFurnishingStatus: value,
                                    isFurnished: value === 'FURNISHED',
                                  })}
                                  className={`min-h-12 rounded-2xl border px-3 text-xs font-bold transition ${
                                    rentalFurnishingStatus === value
                          ...
                          <fieldset>
                            <legend className="text-[11px] font-extrabold text-slate-700">Servicios incluidos en la mensualidad</legend>
                            <p className="mt-1 text-[10px] text-slate-500">Selecciona cada servicio por separado.</p>
                            <div className="mt-2 grid gap-2 sm:grid-cols-3">
                              {([
                                ['WATER', 'Agua'],
                                ['ELECTRICITY', 'Luz'],
                                ...
                                const checked = includedRentalServices.includes(value);
                                return (
                                  <button
                                    key={value}
                                    type="button"
                                    role="checkbox"
                                    aria-checked={checked}
                                    onClick={() => toggleIncludedRentalService(value)}
                                    className={`flex min-h-12 items-center gap-3 rounded-2xl border px-4 text-left text-xs font-bold transition ${
                                      checked
                                        ? 'border-violet-300 bg-violet-50 text-violet-950'
                                        : 'border-slate-200 bg-white text-slate-700 hover:border-slate-400'
                                    }`}
                                  >
                                    <span className={`flex h-5 w-5 items-center justify-center rounded-md border ${checked ? 'border-violet-600 bg-violet-600 text-white' : 'border-slate-300'}`}>
                          ...
                          <Field label="Reglas del inmueble" hint="Condiciones específicas de la renta, una por línea.">
                            <textarea
                              rows={3}
                              value={typeof rentalMetadata.rentRules === 'string' ? rentalMetadata.rentRules : ''}
                              onChange={(event) => updateMonthlyRental({}, { rentRules: event.target.value })}
                              className={`${inputClass} resize-y py-3.5`}
                              placeholder="Ej. No fiestas, horario de ruido, fumar sólo en terraza…"
                            />
                          </Field>
                          ...
                          <div className="rounded-2xl border border-slate-200 bg-white p-4">
                            <div className="flex flex-wrap items-end justify-between gap-2">
                              <div>
                                <p className="text-[11px] font-black text-slate-950">Comisión por renta mensual</p>
                                <p className="mt-1 text-[10px] text-slate-500">Usa el mismo esquema elegido al crear la propiedad.</p>
                              </div>
                              {rentalCommissionModel === 'ONE_MONTH_RENT' && (
                                <span className="rounded-full bg-emerald-50 px-3 py-1 text-[9px] font-black uppercase tracking-wide text-emerald-700">
                                  ${Number(offering.priceAmount || 0).toLocaleString('es-MX')} {offering.currency || 'MXN'}
                                </span>
                              )}
                            </div>
                            <div className="mt-3 grid gap-2 sm:grid-cols-2">
                              {([
                                ['ONE_MONTH_RENT', 'Un mes de renta', 'La comisión equivale a una mensualidad.'],
                                ['PERCENTAGE', 'Porcentaje acordado', 'Conserva la captación y comisión compartida.'],
                              ] as const).map(([value, label, description]) => (
                                <button
                                  key={value}
                                  type="button"
                                  aria-pressed={rentalCommissionModel === value}
                                  onClick={() => updateMonthlyRental({}, {
                                    rentalCommissionModel: value,
                                    rentalCommissionMonths: value === 'ONE_MONTH_RENT' ? 1 : null,
                                    rentalCommissionAmount: value === 'ONE_MONTH_RENT' ? offering.priceAmount ?? null : null,
                                  })}
                                ...
                                >
                                  ...
                                  <span className={`mt-1 block text-[9px] leading-relaxed ${rentalCommissionModel === value ? 'text-white/65' : 'text-slate-400'}`}>{description}</span>
                                </button>
                              ))}
                            </div>
                          </div>
              ...
              <Section id="media" eyebrow="06 · Contenido visual" title="Fotos, video y recorridos" description="La primera imagen funciona como portada. Puedes ordenar, reemplazar o eliminar el material existente.">
                ...
                <div className="mt-7 grid gap-5 lg:grid-cols-2">
                  <div><p className="mb-2 text-[11px] font-extrabold text-slate-700">Videos de la propiedad</p><VideoUploadDropzone videoUrls={form.videoUrls} onChange={(urls) => update('videoUrls', urls)} maxVideos={5} /></div>
              ...
              <Section id="legal" eyebrow="07 · Expediente" title="Situación legal y contacto privado" description="Estos datos son de gestión interna. No se muestran públicamente salvo que otra sección lo indique.">
                <div className="grid gap-3 md:grid-cols-2">
                  {([
                    ['Libre de gravamen', 'legalDebtFree', form.legalDebtFree],
                    ['Escritura pública', 'legalPublicDeed', form.legalPublicDeed],
                    ['Predial al corriente', 'legalTaxCurrent', form.legalTaxCurrent],
                    ['Servicios pagados', 'legalServicesPaid', form.legalServicesPaid],
                  ] as const).map(([label, key, value]) => (
                    <Field key={key} label={label} hint="Sólo selecciona “verificado” cuando exista evidencia revisada.">
                      <select
                        value={value == null ? 'UNKNOWN' : value ? 'YES' : 'NO'}
                        onChange={(event) => update(key, event.target.value === 'UNKNOWN' ? null : event.target.value === 'YES')}
                        className={inputClass}
                      >
                        <option value="UNKNOWN">Sin verificar</option>
                        <option value="YES">Sí, verificado</option>
                        <option value="NO">No / incumple</option>
                      </select>
                    </Field>
                  ))}
                  <Field label="Situación hipotecaria" hint="Selecciona el estado actual de la propiedad.">
                    <select
                      value={form.legalIsMortgaged == null ? 'UNKNOWN' : form.legalIsMortgaged ? 'YES' : 'NO'}
                      onChange={(event) => update('legalIsMortgaged', event.target.value === 'UNKNOWN' ? null : event.target.value === 'YES')}
                      className={inputClass}
                    >
                      <option value="YES">Hipoteca activa</option>
                      <option value="NO">Sin hipoteca activa</option>
              ...
              <Section id="publishing" eyebrow="08 · Visibilidad" title="Publicación y buscadores" description="Controla el estado operativo del anuncio y cómo se presenta en resultados de búsqueda.">
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  {isAdmin ? (
                    ...
                    <Field label="Estado del expediente">
                      <div className={`${inputClass} flex items-center bg-amber-50 text-amber-800`}>
                        {form.folderStatus === 'UNDER_REVIEW' ? 'En revisión por Towers México' : form.isPublished ? 'Publicado' : 'Pendiente de aprobación'}
                      </div>
                    </Field>
                  )}
                  <Field label="Folio Towers México" hint="Identificador único, visible y no editable."><input value={form.internalCode || 'Se asignará al guardar'} readOnly className={`${inputClass} cursor-not-allowed bg-slate-100 font-black tracking-wide text-slate-600`} /></Field>
                  {isAdmin ? (
                  ...
                  <Field label="Título SEO" className="md:col-span-2"><input value={form.metaTitle} onChange={(event) => update('metaTitle', event.target.value)} className={inputClass} placeholder={form.title ? `${form.title} | Towers México` : ''} /></Field>
